...
export class AVMAPI extends JRPCAPI {
  ...
  refreshBlockchainID = (blockchainID: string = undefined): boolean => {
    ...
    if (
      ...
      this.blockchainID = Defaults.network[`${netid}`].Swap.blockchainID //default to Swap-Chain
      return true
    }
  ...
  getDefaultTxFee = (): BN => {
    return this.core.getNetworkID() in Defaults.network
      ? new BN(Defaults.network[this.core.getNetworkID()]["Swap"]["txFee"])
      : new BN(0)
  }
  ...
  getDefaultMintTxFee = (): BN => {
    return this.core.getNetworkID() in Defaults.network
      ? new BN(Defaults.network[this.core.getNetworkID()]["Swap"]["mintTxFee"])
      : new BN(0)
  }
