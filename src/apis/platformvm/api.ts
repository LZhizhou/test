...
export class PlatformVMAPI extends JRPCAPI {
  ...
  protected minNominatorStake: BN = undefined
  ...
  refreshBlockchainID = (blockchainID: string = undefined): boolean => {
    ...
    if (
      ...
      this.blockchainID = PlatformChainID //default to Core-Chain
      return true
    }
  ...
  addValidator = async (
    ...
    nominationFeeRate: BN = undefined
  ): Promise<string> => {
  ...
  addNominator = async (
    ...
    const params: AddNominatorParams = {
      username,
      password,
      nodeID,
      startTime: startTime.getTime() / 1000,
  ...
  issueTx = async (tx: string | Buffer | Tx): Promise<string> => {
    ...
    if (typeof tx === "string") {
      ...
      throw new TransactionError(
        "Error - core.issueTx: provided tx is not expected type of string, Buffer, or Tx"
      )
    }
    const params: any = {
  ...
  getMinStake = async (
    ...
    this.minNominatorStake = new BN(response.data.result.minNominatorStake, 10)
    return {
      minValidatorStake: this.minValidatorStake,
      minNominatorStake: this.minNominatorStake
    }
  ...
  setMinStake = (
    ...
    minNominatorStake: BN = undefined
  ): void => {
  ...
  buildImportTx = async (
    ...
    const atomicUTXOs: UTXOSet = await (
      await this.getUTXOs(ownerAddresses, sraxChain, 0, undefined)
    ).utxos
    const axcAssetID: Buffer = await this.getAXCAssetID()
  ...
  buildAddNominatorTx = async (
    ...
    const minStake: BN = (await this.getMinStake())["minNominatorStake"]
    if (stakeAmount.lt(minStake)) {
      throw new StakeError(
        "PlatformVMAPI.buildAddNominatorTx -- stake amount must be at least " +
    ...
    if (startTime.lt(now) || endTime.lte(startTime)) {
      throw new TimeError(
        "PlatformVMAPI.buildAddNominatorTx -- startTime must be in the future and endTime must come after startTime"
      )
    }

    const builtUnsignedTx: UnsignedTx = utxoset.buildAddNominatorTx(
      this.core.getNetworkID(),
      bintools.cb58Decode(this.blockchainID),
      axcAssetID,
      to,
  ...
  buildAddValidatorTx = async (
    ...
    if (
      typeof nominationFee !== "number" ||
      nominationFee > 100 ||
      nominationFee < 0
    ) {
      throw new NominationFeeError(
        "PlatformVMAPI.buildAddValidatorTx -- nominationFee must be a number between 0 and 100"
      )
    }
