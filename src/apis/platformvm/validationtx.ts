...
import { NominationFeeError } from "../../utils/errors"
...
export class AddValidatorTx extends AddNominatorTx {
  ...
  serialize(encoding: SerializedEncoding = "hex"): object {
    ...
    return {
      ...fields,
      nominationFee: serialization.encoder(
        this.getNominationFeeBuffer(),
        encoding,
        "Buffer",
        "decimalString",
        4
  ...
  protected nominationFee: number = 0
   ...
   */
  getNominationFeeBuffer(): Buffer {
    let dBuff: Buffer = Buffer.alloc(4)
    let buffnum: number =
      parseFloat(this.nominationFee.toFixed(4)) *
  ...
  toBuffer(): Buffer {
    ...
    let feeBuff: Buffer = this.getNominationFeeBuffer()
    ...
    if (typeof nominationFee === "number") {
      if (nominationFee >= 0 && nominationFee <= 100) {
        ...
        throw new NominationFeeError(
          "AddValidatorTx.constructor -- nominationFee must be in the range of 0 and 100, inclusively."
        )
      }
    }
  }
